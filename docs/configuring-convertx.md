...
This is an [Ansible](https://www.ansible.com/) role which installs [ConvertX](https://github.com/convertx-org/convertx) to run as a [Docker](https://www.docker.com/) container wrapped in a systemd service.
...
```yaml
convertx_environment_variables_convertx_sfw_only: on
```
...
convertx_environment_variables_convertx_default_theme: system
```
...
convertx_environment_variables_convertx_default_front_page: default
```

To enable blurring NSFW content by default for users, add the following configuration to your `vars.yml` file:

```yaml
convertx_environment_variables_convertx_default_blur_nsfw: on
```
...
See [`.env.example`](https://github.com/convertx-org/convertx/blob/main/.env.example) for a complete list of ConvertX's config options that you could put in `convertx_environment_variables_additional_variables`.
